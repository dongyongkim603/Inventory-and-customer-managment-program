...
/**
 * A Menu class that utilizes DLL to hold values in video store
 * 
 * @author John
 */
public class MenuDLL {
    ...
    private final DoublyLinkedList<video> DLLvideo = new DoublyLinkedList<>();

    // class instance object variable of the DoublyLinkedList data structure with
    // type customer
    private final DoublyLinkedList<CustomerDLL> DLLcustomer = new DoublyLinkedList<>();

    //// class instance object variable of the DoublyLinkedList data structure with
    //// type videoCheck. List all total videos in store records
    private final DoublyLinkedList<video> DLLTotalVideos = new DoublyLinkedList<>();

    public CustomerDLL tempCustomer = new CustomerDLL(); // used as a temporary CustomerDLL object
    ...
    public void addVideo() {
        ...
        DLLvideo.addFirst(video); // adds the newly created videoCheck object to the DLLvideo list
        DLLTotalVideos.addFirst(video); // adds the newly created videoCheck object to the DLLTotalVideos list
    ...
    public void deleteVideo() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        video video = new video<>(name, id); // creates new videoCheck object from name and id variables
        containsVideo(video, DLLvideo); // calls the containsVideo method to return index of videoCheck if exists if not
                                        // return -1
        tempVideo = containsVideo(video, DLLTotalVideos); // calls the containsVideo method to return index of
     ...
     */
    public CustomerDLL addCustomer() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        CustomerDLL customer = new CustomerDLL(name, id); // creates a new customer object from name and id variables
        DLLcustomer.addFirst(customer); // adds newly created customer object to SLL customer list
        System.out.println(customer.getName() + " " + customer.getId() + " has been added");
        return customer;
    }

    /**
     * allows the user to delete customer from DLLcustomer list. If the customer
    ...
    public void deleteCustomer() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        CustomerDLL customer = new CustomerDLL(name, id); // creates new customer object from name and id variables
    ...
    public boolean checkInStore() {
        ...
        tempVideo = containsVideo(video, DLLvideo); // calls containsVideo method to return index of videoCheck if
                                                    // exists if not return-1
        if (tempVideo != null) {
            System.out.println("the video " + name + " " + id + " is in the store");
            DLLvideo.addFirst(tempVideo);
    ...
    public boolean checkInStore(video video) {
        tempVideo = containsVideo(video, DLLTotalVideos); // calls the containsVideo method which returns index of
                                                          // videoCheck if exists if not return -1
        if (tempVideo != null) // if tempVideo does not equal null it was found
        {
            DLLTotalVideos.addFirst(tempVideo); // re-add the tempVideo to the total video list
    ...
    public boolean checkCustomer() {
        ...
        if (tempCustomer != null) {
            ...
            DLLcustomer.addFirst(tempCustomer);
     ...
     * store or DLLvideo list if it does it will then ask user to enter customer
    ...
    public boolean checkOutVideo() {
        ...
        tempVideo = containsVideo(video, DLLvideo); // creates an integer of index of the videoCheck object if found -1
                                                    // if not
        if (tempVideo != null) {
            ...
            String customerId = getInputString().toUpperCase(); // calls getInoutString method and converts to all
                                                                // uppercase
            CustomerDLL customer = new CustomerDLL(customerName, customerId); // creates new customer object from
            ...
            while (tempCustomer == null) // if not enters loop to create new customer to add to DLLcustomer
            {
     ...
     * inform the user that it has been returned and by the corresponding customer.
    ...
    public boolean checkInVideo() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        video video = new video(name, id); // creates new videoCheck object from name and id variables
        tempVideo = containsVideo(video, DLLTotalVideos); // calls the hasVideo method returns the index of customer
                                                          // with assocciated videoCheck
        if (tempVideo != null) // is not a video of our store
        {
            DLLTotalVideos.addFirst(tempVideo); // add tempVideo to the first of total videos
            tempVideo = null; // clear the tempVideo
            int i = DLLcustomer.getSize(); // get the size of the customer list
            if (i != 0) // if there are customers
            {
                while (i != 0) // from number of customers to 0
                {
                    tempCustomer = DLLcustomer.removeFirst(); // set tempCustomer to the removed first customer in list
                    ...
                    while (j != 0) // while customer has videos
                    {
                        ...
                        {
                            DLLvideo.addFirst(tempVideo); // return the video to the store by adding it to the store
                                                          // video list
                            System.out.println("the video " + video.getTitle() + " has been returned");
                            return true; // video was returned return true
                        }
                    }
                    DLLcustomer.addLast(tempCustomer); // customer did not have video re-add to end of list
                    i--; // decrement i
                }
    ...
    public void printAllCustomers() {
        int i = DLLcustomer.getSize(); // the size of the list
        if (i > 0) // if there are customers
        {
            System.out.println("there are " + i + " customer members");
            for (int j = 0; i > j; j++) // for j at 0 to the number of customers
            {
                tempCustomer = DLLcustomer.removeFirst(); // removes first customer tempCustomer is set to return
                System.out.println("Customer Name: " + tempCustomer.getName() + " Customer Id:" + tempCustomer.getId());
                DLLcustomer.addLast(tempCustomer); // re-add customer to list
            }
        } else {
    ...
    public void printALLInStoreVideos() {
        int i = DLLvideo.getSize(); // the size of the list
        if (i > 0) // if there are videos in store list
        {
            System.out.println("there are " + i + " videos in the store");
            for (int j = 0; i > j; j++) {
                tempVideo = DLLvideo.removeFirst(); // removes first item in DLLvideo and sets return to tempVideo
                System.out.println("Video title: " + tempVideo.getTitle() + " video id: " + tempVideo.getId());
                DLLvideo.addLast(tempVideo); // re-adds the video to the end of list
            }
        } else {
    ...
    public void printAllVideos() {
        int i = DLLTotalVideos.getSize(); // the size of the list
        if (i > 0) // SLLTotalVideos is non-empty
        {
            System.out.println("there are " + i + " videos in the store database");
            for (int j = 0; i > j; j++) {
                tempVideo = DLLTotalVideos.removeFirst(); // removes first video in total video list and sets videoTemp
                                                          // as return value
                System.out.println("Video title: " + tempVideo.getTitle() + " video id: " + tempVideo.getId());
                DLLTotalVideos.addLast(tempVideo); // re-adds the video to list
            }
     ...
     * if there are no customers return. else go through the DLLcustomer list. then
    ...
    public void printAllRentedVideo() {
        ...
        if (DLLcustomer.isEmpty()) {
            System.out.println("there are no customers in database");
            return;
        }
        int i = DLLcustomer.getSize(); // number of customers in DLLcustomer list
        while (i != 0) // while the customer index is pointing at customer
        {
            tempCustomer = DLLcustomer.removeFirst();
            if (tempCustomer.numberOfRented != 0) // customer has rented videos
            {
                ...
                {
                    tempVideo = tempCustomer.rentedVideo.removeFirst(); // removes first video from customer DLL
     ...
     * CustomerDLL object. This is passed to containsCustomer method which will
     * return the index of the customer in the DLLcustomer list and a -1 if no such
    ...
    public void printCustomerRental() {
        int i = DLLcustomer.getSize(); // get size of the customer list
        if (i != 0) // if not empty
        {
