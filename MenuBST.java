...
/**
 * A Menu class that utilizes BST trees to hold values in video store
 * 
 * @author John
 */
public class MenuBST {
    ...
    private final TreeMap<String, video> BSTvideo = new TreeMap<>();

    // class instance object variable of the DoublyLinkedList data structure with
    // type customer
    private final TreeMap<String, CustomerBST> BSTcustomer = new TreeMap<>();

    //// class instance object variable of the DoublyLinkedList data structure with
    //// type videoCheck. List all total videos in store records
    private final TreeMap<String, video> BSTTotalVideos = new TreeMap<>();

    protected video comparatorVideo = new video(); // used as a temporary video object
    protected CustomerBST comparatorCustomer = new CustomerBST(); // used as a temporary CustomerBST object
    ...
    public void addVideo() {
        ...
        BSTvideo.put(id, video); // puts video object in in-store tree at key of video id
        BSTTotalVideos.put(id, video); // puts video object in total video tree at key of video id
        System.out.println(video.getId() + " " + video.getTitle() + " has been added!");
    }

    /**
     * allows user to add video object to BST by calling the getInputString method
     * to create name and id String variables and constructing video object. This
     * object is then passed to the get method which will use the String id of the
     * video object as a key to look for the match. comparator is the returned video
     * object null if not found. This object is then compared to a null value and if
     * null the user is informed the video does not exist if not equal to null the
     * id key is used to remove from both BSTvideo and BSTTotalVideos
     */
    public void deleteVideo() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        comparatorVideo = BSTTotalVideos.get(id); // uses get method to look for matching video in tree
        if (comparatorVideo != null) // if the video was found comparator will equal video
        {
            BSTvideo.remove(id); // remove the node at video id in BSTvideo
            BSTTotalVideos.remove(id); // remove the node at video ide in BSTTotalVideos
     ...
     */
    public CustomerBST addCustomer() {
        ...
        String id = getInputString().toUpperCase(); // calls getInoutString method and converts to all uppercase
        CustomerBST customer = new CustomerBST(name, id); // creates a new customer object from name and id variables
        BSTcustomer.put(id, customer); // adds the newly created customer object to the BSTcustomer tree
        System.out.println(customer.getName() + " with ID: " + customer.getId() + " is now a memeber!");
        return customer;
    }

    /**
     * allows the user to delete customer from BSTcustomer tree. If the customer
     * exists the comparator CustomerBST object will not be null since it was found
    ...
    public void deleteCustomer() {
        ...
        {
            BSTcustomer.remove(id); // the customer is then removed from the tree
    ...
    public boolean checkOutVideo() {
        ...
        } else // else video is in store
        {
            ...
            {
                CustomerBST hold = BSTcustomer.remove(comparatorCustomer.getId());// creates new CustomerBST object by
    ...
    public boolean checkInVideo() {
        ...
        } else // there are customers
        {
            ...
            while (j != 0) // while there are customers to be checked
            {
                ...
                comparatorCustomer = BSTcustomer.get(key); // sets the comparatorCustomer to the customer at key in tree
                if (null != comparatorCustomer.rentedVideo.get(id)) // if the customer has the mataching id of the video
                {
                    comparatorVideo = comparatorCustomer.rentedVideo.remove(id);// sets the comparatorVideo variable to
                                                                                ...
                                                                                // customer
                    BSTvideo.put(comparatorVideo.getId(), comparatorVideo); // re-adds the video to the BSTvideo list ie
     ...
     * tree is empty the user is informed. If BSTcustomer is not empty, The tree is
     * traversed from lowest key value to greatest and each customer is printed.
     */
    public void printAllCustomers() {
        if (BSTcustomer.isEmpty()) // the BSTcustomer tree is empty
        {
            System.out.println("Oh NO! There are no customers!!!");
        } else // BSTcustomer is non-empty
        {
            int i = BSTcustomer.size(); // i is set to the size of the tree
            System.out.println("there are " + i + " customers"); // prints the number of customers
            Entry entry = BSTcustomer.firstEntry(); // gets entry of lowest key value
            while (i != 0) // while there are nodes to be traversed
            {
                String key = entry.getKey().toString(); // String of the entry key value
                comparatorCustomer = BSTcustomer.get(key); // set the comparatorCustomer variable to customer in tree at
                                                           // value key
                ...
                entry = BSTcustomer.higherEntry(key); // sets the entry to the next highest key value
                i--; // decrement i
            }
        }
    }
     ...
     * tree is empty the user is informed. If BSTvideo is not empty, The tree is
     * traversed from lowest key value to greatest and each video is printed.
     */
    public void printALLInStoreVideos() {
        if (BSTvideo.isEmpty()) // there are no videos in store
        {
            System.out.println("There are current;y no videos in the store");
        } else // BSTvideo is a non-empty tree
        {
            int i = BSTvideo.size(); // i is set the size of the tree
            Entry entry = BSTvideo.firstEntry(); // an entry object is set to the lowest key value
            ...
            while (i != 0) // while there are nodes to traverse in tree
            {
                ...
                comparatorVideo = BSTvideo.get(key); // the comparatorVideo variable is set to referenece the video at
                                                     // key
                ...
                entry = BSTvideo.higherEntry(key); // the entry is set the entry with the next higest key value
                i--; // decrement i
            }
        }
    }
     ...
     * BSTTotalVideos tree is empty the user is informed. If BSTTotalVideo is not
     * empty, The tree is traversed from lowest key value to greatest and each video
     * is printed.
     */
    public void printAllVideos() {
        if (BSTTotalVideos.isEmpty()) // there are no videos in database
        {
            System.out.println("There are no videos in the store database!!");
        } else // BSTTotalVideos is non-empty
        {
            int i = BSTTotalVideos.size(); // i is set to the size of tree
            Entry entry = BSTTotalVideos.firstEntry(); // entry is set to the lowest key value
            ...
            while (i != 0) // while there are nodes to be traversed
            {
                ...
                comparatorVideo = BSTTotalVideos.get(key); // sets the comparatorVideo to reference entry at key
                ...
                System.out.println("Title: " + title + " ID: " + id);
                entry = BSTTotalVideos.higherEntry(key); // entry is updated to entry with next greates key value
                i--; // decrement i
            }
        }
    }
     ...
     * BSTTotalVideos tree is empty the user is informed. If there are no customers
     * in the BSTcustomer list the user is informed. Else the BSTcustomer tree is
    ...
    public void printAllRentedVideo() {
        if (BSTcustomer.isEmpty()) // there are no customers
        {
            System.out.println("There are no customers to be able to rent any videos!");
        } else if (BSTTotalVideos.isEmpty()) // there are no videos in database
        {
            System.out.println("There are no videos in the database to rent!!");
        } else if (BSTTotalVideos.size() == BSTvideo.size()) {
     ...
     * database, there are less videos in the BSTvideo tree than the BSTTotalVideo
    ...
    public void printCustomerRental() {
        if (BSTcustomer.isEmpty()) // Customer tree is empty
        {
            System.out.println("There are no customers!!!");
        } else if (BSTTotalVideos.isEmpty()) // total video tree is empty therefore no videos to rent
        {
            System.out.println("There are no videos in database to rent!!"); // videos in equal videos total therefore
                                                                             // none are out
        } else if (BSTvideo.size() == BSTTotalVideos.size()) {
            ...
            if (true == checkCustomer()) // method call allows comparatorCustomer to be changed to targer customer
            {
                ...
                comparatorCustomer = BSTcustomer.get(id); // resets the comparator customer to reference the target in
                                                          // tree
